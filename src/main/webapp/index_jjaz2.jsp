...
<div class="container">
	...
	<div class="row" style="border:2px solid red;">
		...
		<button class="btn btn-warning" id="search_btn">查询提货单</button>
	...
	<script type="text/javascript">
		...
		$(function(){
			//去首页
			to_pageWithQuery(1,creator,dateGt,dateLt);
		});
		
		function to_pageWithQuery(pn,creator,dateGt,dateLt){
			$.ajax({
				url:"${APP_PATH}/findByExample",
				url:"${APP_PATH}/findByExample",
				data:"pn="+pn+"&creator=" +creator+"&dateGt="+dateGt+"&dateLt="+dateLt ,
...
function build_orders_table(result){
	...
	var personOneSum = $("<td></td>").append(item.roadpay+item.prepay); //个人单日合计
	...
	$("<tr></tr>").append(checkBoxTd)
		...
		function build_page_nav(result){
			...
			if(result.extend.pageInfo.hasPreviousPage == false){
				...
				prePageLi.click(function(){
					to_pageWithQuery(result.extend.pageInfo.pageNum -1,creator,dateGt,dateLt);
					//to_pageWithQuery();
				});
			}
			...
			if(result.extend.pageInfo.hasNextPage == false){
				...
				lastPageLi.click(function(){
					to_pageWithQuery(result.extend.pageInfo.pages,creator,dateGt,dateLt);
				});
			}
			...
			$.each(result.extend.pageInfo.navigatepageNums,function(index,item){
				...
				numLi.click(function(){
					to_pageWithQuery(item,creator,dateGt,dateLt);
				});
				ul.append(numLi);
			});
			//添加下一页和末页 的提示
		...
		function validate_add_form(){
			//1、拿到要校验的数据，使用正则表达式
			var empName = $("#address_input").val();
			var regName = /(^[a-zA-Z0-9_-]{6,16}$)|(^[\u2E80-\u9FFF]{2,5})/;
			if(!regName.test(empName)){
				//alert("用户名可以是2-5位中文或者6-16位英文和数字的组合");
				show_validate_msg("#address_input", "error", "用户名可以是2-5位中文或者6-16位英文和数字的组合！！！");
				return false;
			}else{
				show_validate_msg("#address_input", "success", "");
			};
			
			//2、校验roadpay信息
			var email = $("#roadpay_add_input").val();
			var regEmail = /^([a-z0-9_\.-]+)@([\da-z\.-]+)\.([a-z\.]{2,6})$/;
			if(!regEmail.test(email)){
				show_validate_msg("#roadpay_add_input", "error", "邮箱格式不正确");
				return false;
			}else{
				show_validate_msg("#roadpay_add_input", "success", "");
			}
			//校验待付款
			/*
			var premoney = $("#prepay_add_input").val();
			var regEmail = /^([a-z0-9_\.-]+)@([\da-z\.-]+)\.([a-z\.]{2,6})$/;
			if(!regEmail.test(email)){
				show_validate_msg("#roadpay_add_input", "error", "邮箱格式不正确");
				return false;
		...
		$(document).on("click",".edit_btn",function(){
			...
			$("#order_update_btn").click(function(){
		        ...
		        $.ajax({
		            ...
		            data:"id="+id+"&createdate="+createdate+"&address="+address+"&roadpay="+roadpay+"&prepay="+prepay+"&creator="+creator,    
		            dataType:"json",
		            success:function (result) {
		            	if(result.code == 100){
							//员工保存成功；
...
// https://blog.csdn.net/qq_33368846/article/details/82223676
        ...
        $("#emp_save_btn").click(function () {
            ...
            $.ajax({
                ...
                type:"post",
                //注意序列化的值一定要放在最前面,并且不需要头部变量,不然获取的值得格式会有问题
                data:"createdate="+createdate+"&address="+address+"&roadpay="+roadpay+"&prepay="+prepay+"&creator="+creator,    
                dataType:"json",
                success:function (result) {
                	if(result.code == 100){
						//员工保存成功；
		...
		$("#sum_raodpay_all_btn").click(function(){
			...
			$.ajax({
				...
				data:"creator=" +creator+"&dateGt="+dateGt+"&dateLt="+dateLt+"&type=1" ,
				type:"GET",
				success:function(result){
		...
		$("#sum_prepay_all_btn").click(function(){
			...
			$.ajax({
				url:"${APP_PATH}/sumPays",
				data:"creator=" +creator+"&dateGt="+dateGt+"&dateLt="+dateLt+"&type=2" ,
				type:"GET",
				success:function(result){
					document.getElementById("hidderOrShow").style.display="";//显示
					$("#resultMsg").val(result.extend.resultMsg);
		...
		$("#sumAll_btn").click(function(){
			...
			$.ajax({
				url:"${APP_PATH}/sumPays",
				data:"creator=" +creator+"&dateGt="+dateGt+"&dateLt="+dateLt+"&type=3" ,
				type:"GET",
				success:function(result){
					document.getElementById("hidderOrShow").style.display="";//显示
					$("#resultMsg").val(result.extend.resultMsg);
